    ...
    <div class="container step-four-file-container mt-5 mb-md-4 py-5">
        ...
        <nav class="mb-4 pt-md-3" aria-label="Breadcrumb">
            <ol class="breadcrumb breadcrumb-dark">
                ...
                <li class="breadcrumb-item active" aria-current="page">My Blocked Yachts</li>
            </ol>
        </nav>
        <!-- Page content-->
        <div class="row">
            ...
            <div class="col-lg-8 col-md-7 mb-5">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <h1 class="h2 text-dark mb-0">My Blocked Yachts</h1>
                </div>
                <!-- Item-->
                @if(isset($boats) && count($boats) > 0)
                    ...
                    @foreach($boats as $boat)
                        <div class="card card-dark card-hover card-horizontal mb-4">
                            <div class="tns-carousel-wrapper card-img-top card-img-hover"><a class="img-overlay" href="javascript:0;"></a>
                                ...
                                @endif
                                @auth
                                    <form action="{{ url('unblocked-yacht') }}"  method="POST">
                                        <div class="content-overlay end-0 top-0 pt-3 pe-3">
                                            @csrf
                                            <input type="hidden" name="boat_slug" value={{ $boat->slug ?? '' }}>
                                            <button class="btn btn-icon btn-light btn-xs text-primary rounded-circle block-boat-button" data-slug="{{ $boat->slug }}" type="submit" data-bs-toggle="tooltip" data-bs-placement="left" title="Unblock this yacht"><i class="fi-eye-on"></i></button>
                                        </div>
                                    </form>
                                @endauth
                            ...
                            <div class="card-body position-relative">
                                <div class="dropdown position-absolute zindex-5 top-0 end-0 mt-3 me-3">
                                    ...
                                    {{-- <ul class="dropdown-menu dropdown-menu-dark my-1" aria-labelledby="contextMenu1">
                                        ...
                                        {{-- @if(!empty($boat->subscription_id) && empty($boat->subscription->ends_at))
                                            <li>
                                                <a href="#cancelSubscriptionModal" data-bs-toggle="modal" class="dropdown-item cancel-subscription" data-subscription-name="{{ $boat->subscription->name }}" type="button"><i class="fi-x me-2"></i>Cancel Subscription</a>
                                            </li>
                                        @endif
                                        <li>
                                ...
                                @if (isset($boat->boat_name) && $boat->boat_name != null)
                                    <h3 class="h6 mb-1"><a class="nav-link-dark" href="javascript:0;">{{ $boat->boat_name ?? '' }}</a></h3>
                                @endif
                                        ...
                                        <div class="col-lg-6 col-xs-12">
                                            ...
                                            @if(count($boat->userPlanAddons) > 0)
                                                @foreach($boat->userPlanAddons as $key => $userAddon)
                                                    @if (isset($addons) && count($addons) > 0 && !empty($addons))
